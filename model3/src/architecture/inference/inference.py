from src.architecture.models.model import Net
...
from src.architecture.dataset.dataset import Dataset
...
def evaluate_model(parameters_path, output_dir, val_csv):
    ...
    dataset_val = Dataset(dataset_root, val_csv, data_transforms['val'])
...
def apply_metrics(data):
    ...
    msd = np.mean(data['distance']**2)
    print(f'Mean squared distance: {msd}')
